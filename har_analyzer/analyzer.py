...
def main(args):
    ...
    for har_file in har_files:
        ...
        # Check if the csv analysis for the interval exists
        if os.path.exists(pages_result_file):
            pages_df = pd.read_csv(pages_result_file, index_col='url')
            entries_df = pd.read_csv(entries_result_file, index_col='url')
        else:
        ...
        for page in tqdm(har_parser.pages):
            # init page variables
            pageUrl = page.url
...
def handle_har_file(interval, har_file, results_dir, pages_result_file, entries_result_file):
    pass
